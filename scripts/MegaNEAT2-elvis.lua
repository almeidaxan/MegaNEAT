...
function computeFitness(rightmost,megamanHP,currentFrame) --HP goes from 0 to 16
    local fitnees =  2 * rightmost + 25 * megamanHP - currentFrame / 2 + 50 * Score -- from 17 generation to 29 generation
    local add1 = - 150 * (16 - megamanHP) + 50 * Score; -- from 30 generation to
    local add2 = 200 * Score

    add2 = add2 + 100 * Score
    if rightmost >= 2400 then
       -- console.write("ENTER HERE " .. rightmost.."DD")
        add2 = add2 + 200 * Score
    end

    return fitnees + add1 + add2
...
while true do
	...
	if controller ~= nil then
        jump = controller["P1 B"]
        rigth = controller["P1 Right"]
        shot = controller["P1 Y"]
	end
	...
	if megamanX > rightmost then
        ...
        if timeout <= 5 and maximumJumps > 0 and jump and rigth then
            timeout = TimeoutConstant
            maximumJumps  = maximumJumps  - 1
            --console.write("jumps = " ..maximumJumps)
        else
            if disparosmegaman > 0 and shot then
                timeout = TimeoutConstant
                disparosmegaman = disparosmegaman - 1
                --console.write("disparos = " ..disparosmegaman)
            end
        end
	end
	...
	if timeout + timeoutBonus <= 0  or megamanHP <= 0 then
        if megamanHP <= 0 then
            console.write("MEGAMA DEAD by HP!!" .. megamanHP)
        else
            console.write("MEGAMA DEAD by TIME!!")
        end
